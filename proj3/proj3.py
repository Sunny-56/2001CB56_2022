...
def proj_psat_gui():
	...
	st.number_input('constant_fk2d',0.75)
	st.number_input('multiplying_factor_3d',0.5)
	st.number_input('Shear_velocity',2.6**3)

	options=st.selectbox('options are',['1. C','2. S','3. A','4. C & S','5. C & A','6. S & A','7. C & S & A','8. all combine'])
	tch = st.number_input('Chose Filtering Method From Above:')
	if tch == 1:
		corr = st.number_input('Enter thresold value C:')
	elif tch == 2:
		SNR = st.number_input('Enter thresold value S:')
	elif tch == 3:
		Lambda = st.number_input('Enter Lambda value for A:',format='%.2f')
	...
	opt=st.selectbox('choose one options',['1. previous point','2. 2*last-2nd_last','3. overall_mean','4. 12_point_strategy','5. mean of previous 2 point', '6. all seqential','7. all parallel'])
